function sparseNoiseExpByChan_old(key, RFtwin, zcutoff, file_ext, plotFlag, figDir, matDir, saveRaw) %, loadMatFile)
...
if nargin < 6  || isempty(figDir)
    ...
    figDir = fullfile(figDir, sprintf('z_co%d', zcutoff)); 
...
if nargin < 7  || isempty(matDir)
    ...
    matDir = fullfile(matDir, sprintf('z_co%d', zcutoff)); 
...
for ikey = 1 : nexp
    ...
    temp_lfp = squeeze(TensorBPTime(3, inf, 1250, reshape(temp_lfp, [1 size(temp_lfp)])));
