...
%
z_cos = [8 10 20]; %[10 8 20]; %[8,10,20]; *8,10 already running (mcmc, positive)
changeZpos = false; % for plotting on the monitor/dont need to set this

for ibase = 16:19 %1:numel(basenames)
    for z_co = z_cos
        ...
        elseif onlyPlotFits  % only plot autoFits (based off directory)
            ...
            for irf = 1:numel(rfType)
                ...
                for ich = 1:nch
                    ...
                    elseif rsq_co > 0.5 && R2 > 0.5
                        title(h(ich),mytitle, 'color', 'g');
                    elseif rsq_co > 0.4 && R2 > 0.4
                        title(h(ich),mytitle, 'color', 'c');
                    elseif R2 > rsq_co
