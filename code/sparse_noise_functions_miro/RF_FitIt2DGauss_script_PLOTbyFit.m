...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%%%% not plotting Ntrs1-Cre_0080s07 %%%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
z_cos = [10]; %[10 8 20]; %[8,10,20]; *8,10 already running (mcmc, positive)
changeZpos = false; % for plotting on the monitor/dont need to set this

fitDirs = cellfun(@(fit) fullfile(muaDir, fit), {'fitGauss2D_chol','fitGauss2D_tilted'}, 'uniformoutput', 0);
fitLabel = {'chol', 'rot'};
for ibase = 1:numel(basenames)
    for z_co = z_cos
        for ifit = 1:numel(fitDirs);
            fitDir = fitDirs{ifit};
            ...
            if ~onlyPlotFits
                ...
                for ich = 1:nch
                    for irf = 1:numel(rfType)
                        ...
                        [~, pars] = fitit('TwoDGaussian', z, ...
                            pars_lb, pars_0, pars_ub, fitit_opts,...
                            [{RF.intRegionX} {RF.intRegionY}]);
                        [fit,C] = TwoDGaussian(pars,[{RF.intRegionX}, {RF.intRegionY}]);
                        SSE = sum((z(:)-fit(:)).^2);
                        ...
                        t = linspace(0, 2*pi,n);
                        xy = [cos(t); sin(t)];
                        k = sqrt(chi2inv(0.95,2)); % ellipse (xsection) at 95% CI
                        w = (k* U *sqrt(D)) * xy;
                        xy_elipse = repmat(mu, [1 n]) + w;
                        ...
                        fitStruct.r2 = r2;
                        fitStruct.sse = SSE;
                        fitStruct.x_fit = xy_elipse(1,:);
                        fitStruct.y_fit = xy_elipse(2,:);
                        fitStruct.x0 = pars(1);
                        fitStruct.y0 = pars(2);
                        fitStruct.C = C;
            ...
            elseif onlyPlotFits  % only plot autoFits (based off directory)
                ...
                for irf = 1:numel(rfType)
                    ...
                    figure(figPars, 'Name', sprintf('%s %s %s',fitLabel{ifit}, basenames{ibase}, thisRF), ...
                        'position', [rfPos(irf) zPos(ifit), scrnfigw,scrnfigh])
