...
% use chol decomp fitting method (fitGuass2D_titled might also be fine)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
plotSeriesStruct = false; % does this with different Rsq options, see below
% for visualization
setFigShit
zPos = [33, 16, 0];
rfPos = [0 30 60];
...
if makeSumStruct || makeSeriesStruct
    for rsq_co = [0.40 0.45 0.50 0.60];
        ...
        sumSerDir = fullfile(sumDir,'seriesFits');
            ...
            for imouse = 1:numel(unqMice)
                ...
                for iser = 1:numel(mouseSeries)
                    ...
                    for ibase = 1:nbase
                        ...
                        if ismatrix(goodRFcounts_all) && size(goodRFcounts_all,2) == 1 % squeezed too hard :(
                            goodRFcounts_all = goodRFcounts_all';
                        end
